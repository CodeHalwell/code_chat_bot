...
from document_loader import VectorDatabase
...
# The following is a dictionary of models and their respective descriptions and pricing.
cost = {
    "open-mistral-7b": {
...
def perform_vector_db_search(file_extentsion, query, k=10):
    """
    Perform a search in the vector database based on the file extension and query
    :param file_extentsion:
    :param query:
    :param k:
    :return: Returns the search results
    """
    if file_extentsion == "pdf":
        document_loader = DocumentLoader()
        clean_text = document_loader.load_pdf('./upload_docs/file.pdf')
    elif file_extentsion == "text":
        document_loader = DocumentLoader()
        clean_text = document_loader.load_text('./upload_docs/file.txt')
    elif file_extentsion == "csv":
        document_loader = DocumentLoader()
        clean_text = document_loader.load_csv('./upload_docs/file.csv')
    else:
        document_loader = DocumentLoader()
        clean_text = document_loader.load_web(url)
    vector_database = VectorDatabase()
    vector_store = vector_database.embed_upload("document_vector_db", clean_text)
    search_results = vector_database.search_vector_db(vector_store, query, k)
...
try:
    ...
    if ai_provider == "OpenAI":
        openai_models = ['gpt-4', "gpt-4-turbo-2024-04-09", "gpt-3.5-turbo-0125", "gpt-3.5-turbo-instuct"]
    ...
    if system_prompt == "Code Assistant":
        ...
        pre_prompt = "Please provide feedback on the query and please return the references used from the following information extracted from the document:  \n"
...
if ai_provider == 'OpenAI':
    ...
    try:
        if prompt := st.chat_input("What is up?"):
            if url:
                pre_prompt = pre_prompt + str(perform_vector_db_search("web", prompt))
            elif text_file:
                pre_prompt = pre_prompt + str(perform_vector_db_search("text", prompt))
            elif csv_file:
                pre_prompt = pre_prompt + str(perform_vector_db_search("csv", prompt))
            elif pdf_file:
                pre_prompt = pre_prompt + str(perform_vector_db_search( "pdf", prompt))
            else:
